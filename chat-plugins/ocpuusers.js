...
try {
	Object.assign(OCPU, {
		...
		removeAllMoney: function (user, punisher) {
			user = toId(user);
			if (!OCPU.userData[user] || OCPU.userData[user].money === 0) return false;
			let data = this.checkExisting(user);
			...
			if (staff) staff.add(`|c|~|[BucksMonitor] ${user} has lost ${oldMoney} buck${OCPU.pluralFormat(oldMoney)} due to being globally locked and/or banned by ${punisher}.`).update();
		},
		...
		moneyCirculating: function () {
			let data = Object.keys(OCPU.userData);
			// [total bucks, average bucks, number of users with at least 1 money]
			let results = [0, 0, 0];

			data.forEach(user => {
				if (OCPU.userData[user].money > 0) {
					if (isNaN(OCPU.userData[user].money) || OCPU.userData[user].money === null) return;
					results[0] += Math.round(Number(OCPU.userData[user].money));
					results[2]++;
				}
			});
		...
		richestUsers: function (number) { // basically the heavy lifting for the command
			number = number > OCPU.moneyCirculating()[2] ? OCPU.moneyCirculating()[2] : number;
			let data = Object.keys(OCPU.userData);
			...
			data.forEach(user => {
				if (OCPU.userData[user].money > 0) {
					if (OCPU.userData[user].money === null || isNaN(OCPU.userData[user].money)) return; // this should never happen... *coughs*
					userids.push(new ResultsArray(user, OCPU.userData[user].money));
				}
			});
		...
		createTell: function (user, reciever, message) { // heavy lifting for /tell
			...
			user = toId(user);
			message = OCPU.emoticons.processEmoticons(Chat.escapeHTML(message)).replace(/&#x2f;/g, '/');
		...
		checkTells: function (user) {
			...
			if (data) {
				if (Object.keys(data.tells).length > 0) {
					for (let i in OCPU.userData[toId(user)].tells) {
		...
		tellStaff: function (from, message, upper) {
			...
			fs.readFile('config/usergroups.csv', 'utf8', function (err, data) {
				...
				staff.forEach(member => {
					if (toId(member) !== toId(from)) OCPU.createTell(from, member, message);
				});
			});
		},
		...
		generateNews: function () {
			...
			Object.keys(news).forEach(announcement => {
				newsDisplay.push(`<h4>${announcement}</h4>${news[announcement].desc}<br /><br /><strong>—<font color="${OCPU.hashColor(news[announcement].by)}">${news[announcement].by}</font></strong> on ${moment(news[announcement].posted).format("MMM D, YYYY")}`);
			});
			return newsDisplay;
		},
		newsDisplay: function (user) {
			...
			if (newsDis.length > 0) {
				...
				return Users(user).send(`|pm| OCPU News|${Users(user).getIdentity()}|/raw ${newsDis}`);
			}
		},
		...
		pmAll: function (message, pmName) {
			if (!pmName) pmName = '~OCPU Server [Do not reply]';
			Users.users.forEach(curUser => {
				curUser.send('|pm|' + pmName + '|' + curUser.getIdentity() + '|' + message);
			});
		},
		pmAllL: function (message, pmName) { //This is a special method only used for the lockdown server command to PM everyone about the alert that does not include Staff.
			if (!pmName) pmName = '~Server Alert System';
			Users.users.forEach(curUser => {
				if (!curUser.isStaff) {
					curUser.send('|pm|' + pmName + '|' + curUser.getIdentity() + '|' + message);
				};
		...
		pmStaffL: function (message, pmName) { //This is a special method only used for the lockdown server command to PM all staff a special set of directions.
			if (!pmName) pmName = '~Server Alert System (staff)';
			Users.users.forEach(curUser => {
				if (curUser.isStaff) {
					curUser.send('|pm|' + pmName + '|' + curUser.getIdentity() + '|' + message);
				}
		...
		pmStaffE: function (message, pmName) { //This is a special method only used for the emrgency server command to PM all staff a special set of directions.
			if (!pmName) pmName = '~Server Alert System (staff)';
			Users.users.forEach(curUser => {
				if (curUser.isStaff) {
					curUser.send('|pm|' + pmName + '|' + curUser.getIdentity() + '|' + message);
