 ...
 * Functions for a lot of this can be found in: ./chat-plugins/OCPUusers.js
 *
 * Credits to Gold - http://gold.psim.us
 *
 * @license MIT license
 */
'use strict';
...
exports.commands = {
	shop: function (target, room, user) {
		...
		if (room.id === 'lobby' && this.broadcasting) {
			return this.sendReplyBox('<center>Click <button name="send" value="/shop" style="background-color: green; font-color: white;" title="Enter the Shop!"><font color="blue"><b>here</button></b></font> to enter our shop!');
		} else {
			updatePrices();
	...
	buy: function (target, room, user) {
		...
		function moneyCheck(price) {
			...
			if (OCPU.readMoney(user.userid) < price) return false;
			if (OCPU.readMoney(user.userid) >= price) return true;
		}
		function alertStaff(message, staffRoom) {
			OCPU.pmUpperStaff('/raw ' + message, '~Server', false);
			if (staffRoom) {
		...
		function processPurchase(price, item, desc) {
			if (!desc) desc = '';
			if (OCPU.readMoney(user.userid) < price && price !== 'FREE') return false; // this should never happen
			if (price !== 'FREE') OCPU.updateMoney(user.userid, -price);
		...
		case 'symbol':
			price = prices['symbol'];
			if (OCPU.hasVip(user.userid)) return this.errorReply("You are a VIP user - you do not need to buy custom symbols from the shop.  Use /customsymbol to change your symbol.");
		...
		case 'customavatar':
			price = prices['custom'];
			if (OCPU.hasVip(user.userid)) return this.errorReply("You are a VIP user - you do not need to buy avatars from the shop.  Use /customavatar to change your avatar.");
			if (!moneyCheck(price)) return this.errorReply("You do not have enough bucks for this item at this time, sorry.");
		...
		case 'animated':
			...
			if (OCPU.hasVip(user.userid)) return this.errorReply("You are a VIP user - you do not need to buy animated avatars from the shop.  Use /customavatar to change your avatar.");
			if (!moneyCheck(price)) return this.errorReply("You do not have enough bucks for this item at this time, sorry.");
		...
		case 'declare':
			...
			if (!moneyCheck(price)) return this.errorReply("You do not have enough bucks for this item at this time, sorry.");
			processPurchase(price, parts[0]);
			alertStaff(OCPU.nameColor(user.name, true) + ' has purchased the ability to declare from the shop.', true);
		...
		*/
		...
		case 'icon':
			...
			processPurchase(price, parts[0], 'Image: ' + parts[1]);
			alertStaff(OCPU.nameColor(user.name, true) + ' has purchased a custom userlist icon. Image: ' + link(parts[1].replace(' ', ''), 'desired icon'), true);
		...
		case 'vipstatus':
			...
			if (!moneyCheck(price)) return this.errorReply("You do not have enough bucks for this item at this time, sorry.");
			processPurchase(price, parts[0]);
			OCPU.modifyBadge(user.userid, 'vip', 'GIVE');
			let vipUsername = toId(target);
			alertStaff(user.name + " has purchased VIP Status from the shop and they have recieved it automatically from the server.", true);
			break;
		...
		case 'magicpack':
			...
			// good
			case '100 bucks':
				OCPU.updateMoney(user.userid, 100);
				break;
			case '125 bucks':
				OCPU.updateMoney(user.userid, 125);
				break;
			case 'the cost of the mystery box back':
				OCPU.updateMoney(user.userid, prices['pack']);
				break;
			...
			case 'ability to set the PotD':
				alertStaff(OCPU.nameColor(user.name, true) + " has won an " + prize + ". Please PM them to proceed with giving them this.", true);
				break;
			...
			Rooms('lobby').add("|raw|" + OCPU.nameColor(user.name, true) + " has bought a Magic Pack from the shop! " + (goodBad === 'good' ? "They have won a(n) <b>" + prize + "</b>!" : "Oh no!  They got a " + prize + " from their pack :(")).update();
			break;
	...
	removebucks: function (target, room, user) {
		...
		if (targetUser.length < 1 || toId(targetUser).length > 18) return this.errorReply("Usernames cannot be this length.");
		let amount = Math.round(Number(toId(parts[1])));
		if (amount > OCPU.readMoney(targetUser)) return this.errorReply("You cannot remove more bucks than the user has.");
	...
	transferbucks: function (target, room, user) {
		...
		if (OCPU.readMoney(user.userid) < amount) return this.errorReply("You cannot transfer more than you have.");

		//finally, transfer the bucks
		OCPU.updateMoney(user.userid, Number(-amount));
	...
	richestuser: function (target, room, user) {
		...
		return this.sendReplyBox(OCPU.richestUsers(number));
	},
	...
	customsymbol: function (target, room, user) {
		if (!user.canCustomSymbol && !OCPU.hasVip(user.userid)) return this.errorReply("You don't have the permission to use this command.");
	...
	economy: function (target, room, user) {
		if (!this.runBroadcast()) return;
		let econ = OCPU.moneyCirculating();
