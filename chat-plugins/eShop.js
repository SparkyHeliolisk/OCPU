...
function writeShop() {
	...
	fs.writeFile('config/eShop.json', JSON.stringify(OCPU.eShop));
}
...
function shopDisplay() {
	...
	for (let i in OCPU.eShop) {
		if (!OCPU.eShop[i]) continue;
		output += '<tr><td style="border: 2px solid #070e96; width: 20%; text-align: center"><button name="send" value="/eshop buy ' + OCPU.eShop[i].id + '">' + OCPU.eShop[i].name + '</button></td><td style="border: 2px solid #070e96; width: 70%; text-align: center">' + OCPU.eShop[i].desc + '</td><td style="border: 2px solid #070e96; width: 10%; text-align: center">' + OCPU.eShop[i].price + '</td></tr>';
	}
	output += '</table></div>';
	return output;
}
...
exports.commands = {
	...
	eshop: {
		add: function (target, room, user, connection, cmd, message) {
			...
			if (OCPU.eShop.closed) return this.sendReply('An error closed the shop.');
			target = target.split(',');
			if (!target[2]) return this.parse('/eshop help');
			if (OCPU.eShop[toId(target[0])]) return this.errorReply(target[0] + ' is already in the shop.');
			...
			OCPU.eShop[toId(target[0])] = new NewItem(target[0], target[1], target[2], isSSB);
			writeShop();
		...
		remove: function (target, room, user, connection, cmd, message) {
			...
			if (!OCPU.eShop[toId(target)]) return this.errorReply(target + ' is not in the shop.');
			delete OCPU.eShop[toId(target)];
			writeShop();
		...
		buy: function (target, room, user, connection, cmd, message) {
			...
			if (OCPU.eShop.closed) return this.sendReply('The shop is closed, come back later.');
			if (!OCPU.eShop[toId(target)]) return this.errorReply('Item ' + target + ' not found.');
			let item = OCPU.eShop[toId(target)];
			Economy.readMoney(user.userid, userMoney => {
				...
				if (item.isSSB) {
					...
					switch (item.id) {
					case 'shiny':
						if (OCPU.ssb[user.userid].canShiny) return this.sendReply('You already own this.');
						break;
					case 'ffacustomsymbol':
						if (OCPU.ssb[user.userid].cSymbol) return this.sendReply('You already own this.');
						if (user.isStaff || user.group === '+') {
							//give free
							OCPU.ssb[user.userid].cSymbol = true;
							return this.sendReply('Because you have global auth you have been given this for free!');
						}
						break;
					case 'customitem':
						if (OCPU.ssb[user.userid].bought.cItem) return this.sendReply('You already own this.');
						break;
					case 'customability':
						if (OCPU.ssb[user.userid].bought.cAbility) return this.sendReply('You already own this.');
						break;
					case 'custommove':
						if (OCPU.ssb[user.userid].bought.cMove) return this.sendReply('You already own this.');
						break;
					default:
						//Unhandled
					}
				...
				Economy.writeMoney(user.userid, item.price * -1, () => {
					Economy.readMoney(user.userid, amount => {
						...
						case 'ffacustomsymbol':
							...
							break;
						case 'customitem':
							OCPU.ssb[user.userid].bought.cItem = true;
							writeSSB();
							break;
						case 'customability':
							OCPU.ssb[user.userid].bought.cAbility = true;
							writeSSB();
							break;
						case 'custommove':
						...
						default:
							...
							if (tok) {
								...
								OCPU.messageSeniorStaff(user.name + ' has purchased a ' + item.name + '.');
							}
						}
