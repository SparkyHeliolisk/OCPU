...
OCPU.ignoreEmotes = {};
try {
	OCPU.ignoreEmotes = JSON.parse(fs.readFileSync('config/ignoreemotes.json', 'utf8'));
} catch (e) {}
...
function parseEmoticons(message) {
	if (emoteRegex.test(message)) {
		message = OCPU.parseMessage(message).replace(emoteRegex, function (match) {
...
OCPU.parseEmoticons = parseEmoticons;
...
exports.commands = {
	...
	emoticon: {
		add: function (target, room, user) {
			...
			Rooms('upperstaff').add(`|raw|${OCPU.nameColor(user.name, true)} has added the emote "${Chat.escapeHTML(parts[0])}": <img width="40" height="40" src="${parts[1]}">`).update();
			OCPU.messageSeniorStaff(`/html ${OCPU.nameColor(user.name, true)} has added the emote "${Chat.escapeHTML(parts[0])}": <img width="40" height="40" src="${parts[1]}">`);
		},
		...
		ignore: function (target, room, user) {
			if (OCPU.ignoreEmotes[user.userid]) return this.errorReply("You are already ignoring emoticons.");
			OCPU.ignoreEmotes[user.userid] = true;
			fs.writeFileSync('config/ignoreemotes.json', JSON.stringify(OCPU.ignoreEmotes));
			this.sendReply(`You are now ignoring emoticons.`);
		},

		unignore: function (target, room, user) {
			if (!OCPU.ignoreEmotes[user.userid]) return this.errorReply("You aren't ignoring emoticons.");
			delete OCPU.ignoreEmotes[user.userid];
			fs.writeFileSync('config/ignoreemotes.json', JSON.stringify(OCPU.ignoreEmotes));
