...
exports.commands = {
	...
	emoticon: function (target, room, user) {
		...
		case 'add':
			...
			Rooms('upperstaff').add('|raw|' + OCPU.nameColor(user.name, true) + ' has added the emote "' + Chat.escapeHTML(parts[1]) +
				'": <img width="40" height="40" src="' + parts[2] + '">').update();
			OCPU.messageSeniorStaff('/html ' + OCPU.nameColor(user.name, true) + ' has added the emote "' + Chat.escapeHTML(parts[1]) +
