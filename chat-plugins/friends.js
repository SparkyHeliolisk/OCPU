...
function checkFriends(user, userid) {
	...
	if (onlineUsers.length > 0) return user.send(`|pm|~OCPU Server|${user.getIdentity()}|Your friends: ${onlineUsers.join(", ")} are online.`);
}
...
exports.commands = {
	...
	friends: {
		...
		list: function (target, room, user) {
			...
			let display = `<div style="max-height: 200px; width: 100%; overflow: scroll;"><table><tr><center><h2>${OCPU.nameColor(target, true, true)}'s Friends List (${friends[friendsId].friendsList.length} Friend${friends[friendsId].friendsList.length > 1 ? "s" : ""}):</h2></center></tr>`;
			friends[friendsId].friendsList.forEach(friend => {
				display += `<tr><td style="border: 2px solid #000000; width: 20%; text-align: center"><button class="button" name="parseCommand" value="/user ${friend}">${OCPU.nameColor(friend, true, true)}</button></td><td style="border: 2px solid #000000; width: 20%; text-align: center"> Last Seen: ${getLastSeen(friend)}</td>`;
