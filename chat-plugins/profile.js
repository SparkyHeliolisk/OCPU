...
function showTitle(userid) {
	...
	if (Db.customtitles.has(userid)) {
		return '<font color="' + Db.customtitles.get(userid)[1] +
			'">(<strong>' + Db.customtitles.get(userid)[0] + '</strong>)</font>';
	}
	return '';
}
...
exports.commands = {
	...
	dev: {
		give: function (target, room, user) {
			...
			this.sendReply('|html|' + OCPU.nameColor(devUsername, true) + " has been given DEV status.");
		...
		take: function (target, room, user) {
			...
			this.sendReply("|html|" + OCPU.nameColor(devUsername, true) + " has been demoted from DEV status.");
	...
	customtitle: {
		...
		give: function (target, room, user) {
			...
			if (Users.get(targetUser)) {
				Users(targetUser).popup(
					'|html|You have received a custom title from ' + OCPU.nameColor(user.name, true) + '.' +
		...
		remove: function (target, room, user) {
			...
			if (Users.get(userid)) {
				Users(userid).popup(
					'|html|' + OCPU.nameColor(user.name, true) + " has removed your custom title."
				);
			}
	...
	profile: function (target, room, user) {
		...
		function showProfile() {
			Economy.readMoney(toId(username), currency => {
				...
				profile += '&nbsp;<font color="#24678d"><strong>Name:</strong></font> ' + OCPU.nameColor(username, true) + '&nbsp;' + getFlag(toId(username)) + ' ' + showTitle(username) + '<br />';
				...
				if (Db.friendcodes.has(toId(username))) {
					profile += '&nbsp;<font color="#24678d"><strong>Friend Code:</strong></font> ' + Db.friendcodes.get(toId(username));
				}
