...
global.writeSSB = function () {
	...
	fs.writeFile('config/ssb.json', JSON.stringify(OCPU.ssb));
};
...
function buildMenu(userid) {
	if (!OCPU.ssb[userid]) return '<span style="color:red"><b>Error: </b>User "' + userid + '" not found in ssb.</span>';
	let speciesName = toId(OCPU.ssb[userid].species);
	/*if (speciesName.substring(0, 8) === 'oricorio') {
	  speciesName = 'oricorio-' + toId(speciesName.substring(8));
	}*/
	let split = OCPU.ssb[userid].species.split('-');
	if (split.length > 1) {
		speciesName = toId(split[0]) + '-' + speciesName.substring(toId(split[0]).length);
	}
	let output = '';
	output += '<div class="setchart" style="height: 155px; background-image:url(//play.pokemonshowdown.com/sprites/' + (Dex.getTemplate(toId(OCPU.ssb[userid].species)).gen === 7 ? 'bw' : 'xydex') + '' + (OCPU.ssb[userid].shiny ? '-shiny' : '') + '/' + speciesName + '.png); background-position: -2px -3px; background-repeat: no-repeat;">';
	output += '<div class="setcol setcol-icon"><div class="setcell-sprite"></div><div class="setcell setcell-pokemon"><label>Pokémon</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/ssb edit species">' + OCPU.ssb[userid].species + '</button></div></div>';
	output += '<div class="setcol setcol-details"><div class="setrow"><div class="setcell setcell-details"><label>Details</label><button class="textbox setdetails" tabindex="-1" name="send" value="/ssb edit details"><span class="detailcell detailcell-first"><label>Level</label>' + OCPU.ssb[userid].level + '</span><span class="detailcell"><label>Gender</label>' + (OCPU.ssb[userid].gender === 'random' ? '-' : OCPU.ssb[userid].gender) + '</span><span class="detailcell"><label>Happiness</label>' + OCPU.ssb[userid].happiness + '</span><span class="detailcell"><label>Shiny</label>' + (OCPU.ssb[userid].shiny ? 'Yes' : 'No') + '</span></button><span class="itemicon" style="background: none"></span></div></div><div class="setrow"><div class="setcell setcell-item"><label>Item</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/ssb edit item">' + (OCPU.ssb[userid].item ? OCPU.ssb[userid].item : '') + '</button></div><div class="setcell setcell-ability"><label>Ability</label><button class="textbox chartinput" style="width:104px; height: 20px; text-align: left" name="send" value="/ssb edit ability">' + OCPU.ssb[userid].ability + '</button></div></div></div>';
	output += '<div class="setcol setcol-moves"><div class="setcell"><label>Moves</label><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (OCPU.ssb[userid].movepool[0] ? OCPU.ssb[userid].movepool[0] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (OCPU.ssb[userid].movepool[1] ? OCPU.ssb[userid].movepool[1] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (OCPU.ssb[userid].movepool[2] ? OCPU.ssb[userid].movepool[2] : '') + '</button></div><div class="setcell"><button class="textbox chartinput" style="width:129px; height: 20px; text-align: left; overflow: hidden" name="send" value="/ssb edit move">' + (OCPU.ssb[userid].cMove ? OCPU.ssb[userid].cMove : (OCPU.ssb[userid].movepool[3] ? OCPU.ssb[userid].movepool[3] : '')) + '</button></div></div>';
	...
	for (let i = 0; i < statNames.length; i++) {
		stats[toId(statNames[i])] = getStat(toId(statNames[i]), {
			species: OCPU.ssb[userid].species,
			evs: OCPU.ssb[userid].evs,
			ivs: OCPU.ssb[userid].ivs,
			nature: OCPU.ssb[userid].nature,
			level: OCPU.ssb[userid].level,
		});
		let evBuf = '<em>' + (OCPU.ssb[userid].evs[toId(statNames[i])] === 0 ? '' : OCPU.ssb[userid].evs[toId(statNames[i])]) + '</em>';
		if (Dex.getNature(OCPU.ssb[userid].nature).plus === toId(statNames[i])) {
			evBuf += '<small>+</small>';
		} else if (Dex.getNature(OCPU.ssb[userid].nature).minus === toId(statNames[i])) {
			evBuf += '<small>&minus;</small>';
		}
		let width = stats[toId(statNames[i])] * 75 / 504;
		if (statNames[i] === 'HP') width = stats[toId(statNames[i])] * 75 / 704;
		if (width > 75) width = 75;
		let color = Math.floor(OCPU.ssb[userid].evs[toId(statNames[i])] * 180 / 714);
	...
	output += '<div style="text-align:center"><button class="button" name="send" value="/ssb custom">Custom Move List</button> | <button class="button" name="send" value="/ssb toggle">' + (OCPU.ssb[userid].active ? 'Deactivate your pokemon' : 'Activate your pokemon') + '</button></div></div>';
	return output;
}
...
function moveMenu(userid) {
	...
	for (let i = 0; i < OCPU.ssb[userid].movepool.length; i++) {
		if (OCPU.ssb[userid].movepool.length === 0) break;
		output += ((i + 1 === OCPU.ssb[userid].movepool.length && !OCPU.ssb[userid].cMove) ? OCPU.ssb[userid].movepool[i] : OCPU.ssb[userid].movepool[i] + ', ');
	}
	if (OCPU.ssb[userid].cMove) output += OCPU.ssb[userid].cMove;
	output += '</div><div style="padding-bottom: 2px"><i>Custom-made Custom Move:</i> ' + (OCPU.ssb[userid].selfCustomMove ? OCPU.ssb[userid].selfCustomMove : '<button name="send" value="/shop" class="button">Purchase</button>') + '</div>';
	output += '<button name="send" class="button" value="/ssb edit move help">Set Moves</button> | <button class="button" name="send" value="/ssb custom">Set a Custom Move</button> | <button name="send" class="button" value="/ssb edit moveq custom, ' + (OCPU.ssb[userid].selfCustomMove ? OCPU.ssb[userid].selfCustomMove : '') + '">Set Custom-made Custom Move</button> | <button name="send" class="button" value="/ssb edit main">Main Menu</button></div>';
	return output;
}

function itemMenu(userid) {
	return '<div class="setchart" style="text-align:center"><h3><u>Item Menu</u></h3><div style="padding-bottom: 2px"><i>Current Item:</i> ' + (OCPU.ssb[userid].item ? OCPU.ssb[userid].item : 'None') + '</div><div style="padding-bottom: 2px"><i>Custom Item:</i> ' + (OCPU.ssb[userid].cItem ? OCPU.ssb[userid].cItem : '<button name="send" value="/shop" class="button">Purchase</button>') + '</div><button name="send" class="button" value="/ssb edit item help">Set Item</button> | <button name="send" class="button" value="/ssb edit itemq reset">Reset Item</button> | <button name="send" class="button" value="/ssb edit itemq ' + (OCPU.ssb[userid].cItem ? OCPU.ssb[userid].cItem : 'help') + '">Set Custom Item</button> | <button name="send" class="button" value="/ssb edit main">Main Menu</button></div>';
}

function abilityMenu(userid) {
	let output = '<div class="setchart" style="text-align:center"><h3><u>Ability Menu</u></h3><div style="padding-bottom: 2px"><i>Current Ability:</i> ' + OCPU.ssb[userid].ability + '</div><div style="padding-bottom: 2px"><i>Custom Ability:</i> ' + (OCPU.ssb[userid].cAbility ? OCPU.ssb[userid].cAbility : '<button name="send" value="/shop" class="button">Purchase</button>') + '</div>';
	let pokemon = Dex.getTemplate(OCPU.ssb[userid].species);
	...
	if (OCPU.ssb[userid].cAbility) output += '<button name="send" value="/ssb edit abilityq ' + OCPU.ssb[userid].cAbility + '" class="button">Set to ' + OCPU.ssb[userid].cAbility + '</button> | ';
...
function statMenu(userid) {
	...
	for (let i = 0; i < values.length; i++) {
		output += '<tr><td><button class="button" name="send" value="/ssb edit statsq ev, ' + values[i] + ', 0">Set 0</button></td><th>' + values[i] + ': ' + OCPU.ssb[userid].evs[toId(values[i])] + '</th><td style="border-right:1px solid black"><button class="button" name="send" value="/ssb edit statsq ev, ' + values[i] + ', 252">Set 252</button></td>';
		output += '<td style="border-left:1px solid black"><button class="button" name="send" value="/ssb edit statsq iv, ' + values[i] + ', 0">Set 0</button></td><th>' + values[i] + ': ' + OCPU.ssb[userid].ivs[toId(values[i])] + '</th><td><button class="button" name="send" value="/ssb edit statsq iv, ' + values[i] + ', 31">Set 31</button></td></tr>';
	}
	output += '<div style="float: right; display: inline-block; width: 40%"><b><u>Stat Menu</u></b><br/><br/><button class="button" name="send" value="/ssb edit stats help">Set EVs or IVs to a custom value</button><br/><br/><i>Current Nature:</i> ' + OCPU.ssb[userid].nature + '<br/><br/><button class="button" name="send" value="/ssb edit stats nature help">Set Nature</button><br/><br/><button class="button" name="send" value="/ssb edit main">Main Menu</button></div></div>';
	return output;
}

function detailMenu(userid) {
	let output = '<div class="setchart" style="text-align:center; height:140px"><h3><u>Details Menu</u></h3>';
	output += '<i>Level: </i>' + OCPU.ssb[userid].level + ' | <button name="send" value="/ssb edit detailsq level, 1" class="button">Set to 1</button> <button name="send" value="/ssb edit detailsq level, 50" class="button">Set to 50</button> <button class="button" name="send" value="/ssb edit detailsq level, 100">Set to 100</button><br/>';
	output += '<i>Gender: </i>' + OCPU.ssb[userid].gender + ' | <button name="send" value="/ssb edit detailsq gender, male" class="button">Set to Male</button> <button name="send" value="/ssb edit detailsq gender, female" class="button">Set to Female</button> <button class="button" name="send" value="/ssb edit detailsq gender, random">Set to Random</button> <button name="send" value="/ssb edit detailsq gender, genderless" class="button">Set to Genderless</button><br/>';
	output += '<i>Happiness: </i>' + OCPU.ssb[userid].happiness + ' | <button name="send" value="/ssb edit details happiness, 0" class="button">Set to 0</button> <button class="button" name="send" value="/ssb edit details happiness, 255">Set to 255</button> <button name="send" value="/ssb edit details happiness" class="button">Set to custom value</button><br/>';
	output += '<i>Shiny?:</i> | ' + (OCPU.ssb[userid].canShiny ? '<button name="send" value="/ssb edit details shiny" class="button">Toggle Shiny</button>' : '<button name="send" value="/shop" class="button">Purchase</button>') + ' | <i>Custom Symbol: </i>' + (OCPU.ssb[userid].cSymbol ? ('' + OCPU.ssb[userid].symbol + ' <button class="button" name="send" value="/ssb edit details symbol">Change</button>') : '<button class="button" name="send value="/shop">Purchase</button>') + ' | <button class="button" name="send" value="/ssb edit main">Main Menu</button></div>';
	return output;
}
...
try {
	if (!noRead) {
		...
		for (let key in raw) {
			OCPU.ssb[key] = new SSB(raw[key].userid, raw[key].name);
			for (let key2 in OCPU.ssb[key]) {
				OCPU.ssb[key][key2] = raw[key][key2];
			}
		}
	} else {
		OCPU.ssb = global.ssb = {};
	...
	ssb: {
		edit: {
			...
			'': function (target, room, user, connection, cmd, message) {
				...
				if (!OCPU.ssb[user.userid]) {
					this.sendReply('Could not find your SSB pokemon, creating a new one...');
					OCPU.ssb[user.userid] = new SSB(user.userid, user.name);
					writeSSB();
				}
				let targetUser = OCPU.ssb[user.userid];
		...
		log: function (target, room, user, connection, cmd, message) {
			...
			case 'view':
				...
				if (toId(target[1]) === 'all') {
					...
					for (let i in OCPU.ssb) {
						for (let j in OCPU.ssb[i].bought) {
							let buttons = '<button class="button" name="send" value="/ssb log mark, ' + OCPU.ssb[i].userid + ', ' + j + ', complete">Mark as Complete</button><button class="button" name="send" value="/ssb log mark, ' + OCPU.ssb[i].userid + ', ' + j + ', pending">Mark as Pending</button><button class="button" name="send" value="/ssb log mark, ' + OCPU.ssb[i].userid + ', ' + j + ', remove"><span style="color:red">Remove this purchase</span</button>';
							output += '<tr><td style="border: 1px solid black">' + OCPU.ssb[i].name + '</td><td style="border: 1px solid black">' + j + '</td><td style="border: 1px solid black">' + (OCPU.ssb[i].bought[j] ? (OCPU.ssb[i].bought[j] === 'complete' ? 'Complete' : 'Pending') : 'Removed') + '</td><td style="border: 1px solid black">' + buttons + '</td></tr>';
						}
					}
				} else {
					target[1] = toId(target[1]);
					if (!OCPU.ssb[target[1]]) return this.errorReply(target[1] + ' does not have a SSBFFA pokemon yet.');
					if (user.can('roomowner')) {
						output += '<th style="border: 1px solid black">Options</th><tr/>';
						for (let j in OCPU.ssb[target[1]].bought) {
							let buttons = '<button class="button" name="send" value="/ssb log mark, ' + OCPU.ssb[target[1]].userid + ', ' + j + ', complete">Mark as Complete</button><button class="button" name="send" value="/ssb log mark, ' + OCPU.ssb[target[1]].userid + ', ' + j + ', pending">Mark as Pending</button><button class="button" name="send" value="/ssb log mark, ' + OCPU.ssb[target[1]].userid + ', ' + j + ', remove"><span style="color:red">Remove this purchase</span</button>';
							output += '<tr><td style="border: 1px solid black">' + OCPU.ssb[target[1]].name + '</td><td style="border: 1px solid black">' + j + '</td><td style="border: 1px solid black">' + (OCPU.ssb[target[1]].bought[j] ? (OCPU.ssb[target[1]].bought[j] === 'complete' ? 'Complete' : 'Pending') : 'Removed') + '</td><td style="border: 1px solid black">' + buttons + '</td></tr>';
						}
					} else {
						output += '</tr>';
						for (let j in OCPU.ssb[target[1]].bought) {
							output += '<tr><td style="border: 1px solid black">' + OCPU.ssb[target[1]].name + '</td><td style="border: 1px solid black">' + j + '</td><td style="border: 1px solid black">' + (OCPU.ssb[target[1]].bought[j] ? (OCPU.ssb[target[1]].bought[j] === 'complete' ? 'Complete' : 'Pending') : 'Removed') + '</td></tr>';
						}
					}
				}
			...
			case 'mark':
				...
				if (!OCPU.ssb[target[1]]) return this.errorReply(target[1] + ' does not have a SSBFFA pokemon yet.');
				if (OCPU.ssb[target[1]].bought[target[2]] === undefined) return this.parse('/help ssb log');
				switch (target[3]) {
				case 'complete':
					if (OCPU.ssb[target[1]].bought[target[2]] === target[3]) return this.errorReply(target[1] + '\'s ' + target[2] + ' is already ' + target[3] + '.');
					OCPU.ssb[target[1]].bought[target[2]] = 'complete';
					writeSSB();
					return this.sendReply(target[1] + '\'s ' + target[2] + ' was marked as complete.');
					//break;
				case 'pending':
					if (OCPU.ssb[target[1]].bought[target[2]] === true) return this.errorReply(target[1] + '\'s ' + target[2] + ' is already ' + target[3] + '.');
					OCPU.ssb[target[1]].bought[target[2]] = true;
					writeSSB();
					return this.sendReply(target[1] + '\'s ' + target[2] + ' was marked as pending.');
					//break;
				case 'remove':
					if (OCPU.ssb[target[1]].bought[target[2]] === false) return this.errorReply(target[1] + '\'s ' + target[2] + ' is already removed.');
					...
					OCPU.ssb[target[1]].bought[target[2]] = false;
					writeSSB();
		...
		validate: function (target, room, user, connection, cmd, message) {
			...
			let targetUser = OCPU.ssb[toId(target)];
			...
			if (toId(cmd) !== 'validateall') {
				...
				for (let key in OCPU.ssb) {
					let valid = validate(this, OCPU.ssb[key], true);
					if (!valid) {
						OCPU.ssb[key].active = false;
						if (Users(toId(OCPU.ssb[key].name))) Users(toId(OCPU.ssb[key].name)).popup('Your SSBFFA pokemon was deactivated because it is invalid.');
						writeSSB();
						this.errorReply(OCPU.ssb[key].name + '\'s pokemon was invalid. Invalid parts have been reset and this pokemon was deactivated.');
					}
				}
