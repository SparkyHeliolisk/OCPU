...
function isFvFBattle(p1, p2, id, status, types, score) {
	...
	if (Rooms.global.FvF[factionId] && Rooms.global.FvF[factionId].challenger && Rooms.global.FvF[factionId].challenger === targetFactionid || Rooms.global.FvF[factionId] && Rooms.global.FvF[factionId].challenging && Rooms.global.FvF[factionId].challenging === targetFactionid) {
		...
		if (status === 'start') {
			...
			fvfDisplay(room);
			room.add('|raw|<a href="/' + id + '">The Faction vs Faction battle between ' + OCPU.nameColor(p1, true) + ' (' + Chat.escapeHTML(factions[factionId].name) + ') and ' +
				OCPU.nameColor(p2, true) + ' (' + Chat.escapeHTML(factions[targetFactionid].name) + ') has begun.</a>').update();
			...
			fvfDisplay(room);
			room.add("|raw|The Faction vs Faction battle between " + OCPU.nameColor(p1, true) + " and " + OCPU.nameColor(p2, true) + " ended with a tie. They have to have a rematch.");
			room.update();
		}
	}
}
OCPU.isFvFBattle = isFvFBattle;
...
exports.commands = {
	...
	factions: {
		...
		kick: function (target, room, user) {
			...
			write();
			if (Users(target) && Users(target).connected) Users(target).send("|popup||html|" + OCPU.nameColor(user.name) + " has kicked you from the faction " + Chat.escapeHTML(factions[factionid].name) + ".");
		...
		promote: function (target, room, user) {
			...
			targetUser.send("|popup||html|" + OCPU.nameColor(user.name) + " has set your faction rank in " + Chat.escapeHTML(factions[factionid].name) + " to " + Chat.escapeHTML(rank) + ".");
		...
		demote: function (target, room, user) {
			...
			if (Users(targetUser) && Users(targetUser).connected) {
				Users(targetUser).send("|popup||html|" + OCPU.nameColor(user.name) + " has removed you from the faction rank " + Chat.escapeHTML(rank) + " in " +
