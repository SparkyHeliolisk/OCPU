...
exports.commands = {
	dev: {
		give: function (target, room, user) {
			...
			this.sendReply(`|html|${OCPU.nameColor(target, true)} has been given DEV status.`);
			if (Users.get(devUsername)) Users(devUsername).popup(`|html|You have been given DEV status by ${OCPU.nameColor(user.name, true)}.`);
		},
		...
		take: function (target, room, user) {
			...
			this.sendReply(`|html|${OCPU.nameColor(target, true)} has been demoted from DEV status.`);
			if (Users.get(devUsername)) Users(devUsername).popup(`|html|You have been demoted from DEV status by ${OCPU.nameColor(user.name, true)}.`);
		},
		...
		list: function () {
			...
			Db.devs.keys().forEach(devUser => {
				display.push(OCPU.nameColor(devUser, (Users(devUser) && Users(devUser).connected)));
			});
	...
	vip: {
		give: function (target, room, user) {
			...
			this.sendReply(`|html|${OCPU.nameColor(vipUsername, true)} has been given VIP status.`);
			if (Users.get(vipUsername)) Users(vipUsername).popup(`|html|You have been given VIP status by ${OCPU.nameColor(user.name, true)}.`);
		},
		...
		take: function (target, room, user) {
			...
			this.sendReply(`|html|${OCPU.nameColor(vipUsername, true)} has been demoted from VIP status.`);
			if (Users.get(vipUsername)) Users(vipUsername).popup(`|html|You have been demoted from VIP status by ${OCPU.nameColor(user.name, true)}.`);
		},
	...
	customtitle: {
		...
		give: function (target, room, user) {
			...
			if (Users.get(targetUser)) {
				Users(targetUser).popup(`|html|You have received a custom title from ${OCPU.nameColor(user.name, true)}.<br />Title: ${showTitle(toId(targetUser))}<br />Title Hex Color: ${color}`);
			}
		...
		remove: function (target, room, user) {
			...
			if (Users.get(userid)) {
				Users(userid).popup(`|html|${OCPU.nameColor(user.name, true)} has removed your custom title.`);
			}
	...
	pcolor: {
		...
		remove: function (target, room, user) {
			...
			if (Users.get(userid)) {
				Users(userid).popup(`|html|${OCPU.nameColor(user.name, true)} has removed your profile color.`);
			}
	...
	lastactive: function (target, room, user) {
		...
		return this.sendReplyBox(`${OCPU.nameColor(targetUser, true, true)} was last active <strong>${Chat.toDurationString(Date.now() - targetUser.lastMessageTime)} ago</strong>.`);
	},
